...
5. [`pkgload::load_all()`](https://pkgload.r-lib.org/reference/load_all.html) is not compatible with `instantiate`. This is because `instantiate` relies on a custom `src/install.libs.R` script to compile the models, and `load_all()` does not pick up custom binaries compiled this way. That means `devtools::test()` may not work as expected. Please install your package the standard way, then test with alternative means such as `devtools::check()`, `R CMD check`, or `tinytest`.
